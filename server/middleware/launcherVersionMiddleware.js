    ...
    try {
        ...
        if (!clientVersion) {
            ...
            return res.status(426).json({ 
                message: 'Launcher ของคุณเป็นเวอร์ชันเก่ามาก กรุณาดาวน์โหลดเวอร์ชันใหม่จากเว็บไซต์ (Update Required)' 
            });
        }
        ...
        for (let i = 0; i < Math.max(vClient.length, vMin.length); i++) {
            ...
            if (c < m) {
                return res.status(426).json({ 
                    message: `ต้องใช้ Launcher เวอร์ชัน ${config.minLauncherVersion} ขึ้นไป กรุณาอัปเดต!` 
                });
            }
        }
