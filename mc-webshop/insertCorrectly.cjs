...
const regexInsert = /\{config\.configFileUrl && <p className="mt-2 text-xs text-gray-500">\{config\.configFileUrl\}<\/p>\}\s*<\/div>/;
...
if (regexInsert.test(code)) {
    code = code.replace(regexInsert, insertStr);
    fs.writeFileSync(filePath, code, 'utf8');
    console.log('Success inserted correctly!');
} else {
    console.log('Not found for insertion');
}
